...
dither_D1 (IMGINFO info, void * _dst)
{
	...
	UWORD   chunk = 0;
	do {
		char  * rgb   = (char*)&info->RowBuf[(x >>16) *3];
		ins += *buf + (WORD)rgb[0] *5 + (WORD)rgb[1] *9 + (WORD)rgb[2] *2;
		if (ins < 2040) {
			chunk |= pixel;
		} else {
...
gscale_I4 (IMGINFO info, void * _dst)
{
	...
	do {
		*(tmp++) = dither_gray ((CHAR*)&info->RowBuf[x >>16], &err, &dth);
		if (!--width || !--n) {
			raster_chunk4 (buf, dst, tmp - buf);
			dst += 4;
			n    = 16;
...
dither_I4 (IMGINFO info, void * _dst)
{
	...
	do {
		*(tmp++) = dither_true ((CHAR*)&info->RowBuf[(x >>16) *3], err, &dth);
		if (!--width || !--n) {
			raster_chunk4 (buf, dst, tmp - buf);
			dst += 4;
			n    = 16;
...
gscale_P8 (IMGINFO info, void * _dst)
{
	...
	do {
		*(dst++) = dither_gray ((CHAR*)&info->RowBuf[x >>16], &err, &dth);
		x += info->IncXfx;
	} while (--width);
}
...
dither_P8 (IMGINFO info, void * _dst)
{
	...
	do {
		*(dst++) = dither_true ((CHAR*)&info->RowBuf[(x >>16) *3], err, &dth);
		x += info->IncXfx;
	} while (--width);
}
